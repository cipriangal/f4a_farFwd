...
void b0tracker()
{
  ...
  Double_t waferThickness        =   1.00; //0.3;
  //Double_t waferWidth            =  350.0;  //NOMINAL USED FOR ALL SIMULATIONS
  Double_t waferWidth            =  260.0;
  Double_t waferHeight           =  270.0;
  ...
  //Double_t waferSpacing          =  (1200.+2*100.)/(waferNum-1); //AYK original spacing, with 2 sensors outside B0magnet
  Double_t waferSpacing          =  (1000.)/(waferNum-1); //AMJ soacing, all sensors within B0 volume
  Double_t beamLineOffsetX       =  190.0;
  Double_t beamLineOffsetZ       = 5900; //new IR  //5600.0;
  ...
  Double_t beam_pipe_clearance       =   8.0; 
  ...
  Double_t nominal_proton_direction  =   .025; //new IR crab rotation angle // 0.0215;
  ...
  for(unsigned wf=0; wf<waferNum; wf++)
  {
    ...
    double r0 = 36.0; //z0*tan(nominal_proton_cone_size) + beam_pipe_clearance;
   
    cout << "beam pipe radius = " << r0 << endl;
    ...
    cout << "z-position: " << (beamLineOffsetZ + dz) << endl;
	cout << "x-position: " << (beamLineOffsetX) << endl;
